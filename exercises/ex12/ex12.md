...
The final value of the counter is correct, but the value that is shown at each
child often is not.
...
Some of the counters are 0 while the values grow a lot at different points, which
implies that it is not running each thread in order, one after the other.
...
a lot of synchronization errors occur (usually > 100,000)
