...
has a lot of advanced data structures, support for threading, standard macros, warnings, and more

glibc is the core C library that has things like printf() and fopen(), that's confusing

*  What are the most important pros and cons of using GLib, as opposed
to another library, or just using core C features?

glib allows you to use a lot more data structures in C without having to implement them,
but you do have to install it separately
...
compile using gcc ex_compile.c -Wall -o ex_compile `pkg-config --cflags --libs glib-2.0`,
tutorial's things do not work
